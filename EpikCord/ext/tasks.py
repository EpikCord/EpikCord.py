...
class TimeParser:
    ...
    @staticmethod
    def get_time_since_epoch(year:int,month:int,day:int,hour:int,minute:int,seconds:float) -> float:
    ...
    @staticmethod
    def total_seconds(year:int=0,month:int=0,day:int=0,hour:int=0,minute:int=0,seconds:float=0) -> float:
        month += year*365
        day += month*30
        hour += day*24
        minute += hour*60
...
class Tasks:
    ...
    def add_task(self, task:Callable[...,None], interval: Optional[int] = None, start:Optional[float]=None, until:Optional[float]=None,**kwargs):
        ...
        task = asyncio.get_event_loop().create_task(full_task(self.client,task, **kwargs))
